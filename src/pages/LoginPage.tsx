...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try { await login(email.trim(), password); toast.success('登录成功'); navigate('/reports', { replace: true }) }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">登录查看你的八字解读</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <p className="text-center text-xs text-slate-400">
            没有账号？<Link to="/register" className="text-purple-600 hover:underline">注册</Link>
          </p>
        </form>
      </div>
    </div>
