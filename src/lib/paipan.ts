...
function parsePillar(str: string) {
  const s = String(str || '').trim()
  return { gan: s.charAt(0), zhi: s.charAt(1), label: s }
}

function applyTrueSolarTime(y: number, m: number, d: number, h: number, min: number, longitude: number) {
...
export function computePaipan(input: PaipanInput) {
  const {
    calendarType = 'solar',
    year,
    month,
  ...
  let adjusted: ReturnType<typeof applyTrueSolarTime>
  ...
  if (calendarType === 'lunar') {
    ...
    adjusted = applyTrueSolarTime(s.getYear(), s.getMonth(), s.getDay(), Number(hour), Number(minute), longitude)
    solar = Solar.fromYmdHms(adjusted.year, adjusted.month, adjusted.day, adjusted.hour, adjusted.minute, 0)
  } else {
    adjusted = applyTrueSolarTime(Number(year), Number(month), Number(day), Number(hour), Number(minute), longitude)
    solar = Solar.fromYmdHms(adjusted.year, adjusted.month, adjusted.day, adjusted.hour, adjusted.minute, 0)
  }
